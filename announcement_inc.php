<?php
      ...
      <div class="d-flex justify-content-end">
          <div class="donate_btn_right">
              <button class="btn now_donate" data-toggle="modal" data-target="#createAnnouncement" title="Create Announcement">
                  <img src="<?php echo get_template_directory_uri(); ?>/assets/images/plus_icon.png"> Create Announcement</button>
          </div>
      </div>
      <div claas="row">
      ...
      <div class="row blog-card-box mt-4">
          ...
          <?php
            if ( !empty( $posts = $group->announcements() ) ) {
                foreach ( $posts as $kcc_post ) {
                     $kcc_post->render_cell();
                     }
                }
            else { ?>
              <?php echo 'No announcements in this group.'; ?>
          <?php } ?>
...
<!-- LOOK HERE -->
      <?
        $form = new \KCC\Forms\AnnouncementPostForm();
